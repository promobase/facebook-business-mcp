...
def extract_manual_sections(file_path: Path) -> dict[str, str]:
    """Extract manual sections from an existing file.
    ...
    pattern = r"# ---- BEGIN MANUAL SECTION: (\w+) ----\n(.*?)\n# ---- END MANUAL SECTION: \1 ----"
...
class PydanticModelGenerator:
    ...
    def generate_model(
        self, adobject_info: AdObjectInfo, existing_file_path: Optional[Path] = None
    ) -> str:
        ...
        if existing_file_path:
            self.manual_sections = extract_manual_sections(existing_file_path)
        else:
        ...
        content = f"{header}{imports_str}\n\n\n{models_str}"

        # Always add manual imports section at the end of imports
        # Find the position after all imports but before the models
        import_end = content.find("\n\n\n")
        if import_end > 0:
            manual_imports_content = self.manual_sections.get("imports", "")
            content = (
                content[:import_end]
                + "\n\n# ---- BEGIN MANUAL SECTION: imports ----\n"
                + manual_imports_content
                + "\n# ---- END MANUAL SECTION: imports ----"
                + content[import_end:]
            )

        # Always add manual end section at the end of file
        manual_end_content = self.manual_sections.get("end", "")
        content += "\n\n\n# ---- BEGIN MANUAL SECTION: end ----\n"
        content += manual_end_content
...
def main():
    ...
    for file_path in files_to_process:
        ...
        if adobject_info and adobject_info.fields:
            ...
            # Generate Pydantic model (passing existing file path for manual sections)
            model_code = generator.generate_model(adobject_info, output_file)
