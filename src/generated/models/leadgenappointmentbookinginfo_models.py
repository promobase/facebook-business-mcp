"""Code generated by PromoBase script - DO NOT EDIT MANUALLY."""

from __future__ import annotations

from typing import Any, Literal

from pydantic import BaseModel, Field

LeadGenAppointmentBookingInfoField = Literal["advertiser_timezone_offset", "appointment_durations", "appointment_slots_by_day"]


class LeadGenAppointmentBookingInfoFields(BaseModel):
    """Pydantic model for LeadGenAppointmentBookingInfo fields."""

    advertiser_timezone_offset: str | None = Field(None, alias="advertiser_timezone_offset")
    appointment_durations: list[str] | None = Field(None, alias="appointment_durations")
    appointment_slots_by_day: list[dict[str, Any]] | None = Field(None, alias="appointment_slots_by_day")

    class Config:
        populate_by_name = True
        extra = 'forbid'