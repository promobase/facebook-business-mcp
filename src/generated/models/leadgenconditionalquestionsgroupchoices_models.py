"""Code generated by PromoBase script - DO NOT EDIT MANUALLY."""

from __future__ import annotations

from typing import Any, Literal

from pydantic import BaseModel, Field

LeadGenConditionalQuestionsGroupChoicesField = Literal["customized_token", "next_question_choices", "value"]


class LeadGenConditionalQuestionsGroupChoicesFields(BaseModel):
    """Pydantic model for LeadGenConditionalQuestionsGroupChoices fields."""

    customized_token: str | None = Field(None, alias="customized_token")
    next_question_choices: list[dict[str, Any]] | None = Field(None, alias="next_question_choices")
    value: str | None = Field(None, alias="value")

    class Config:
        populate_by_name = True
        extra = 'forbid'