"""Code generated from Facebook API specs - DO NOT EDIT MANUALLY."""

from __future__ import annotations

from datetime import datetime
from enum import Enum
from typing import TYPE_CHECKING, Any, Literal

from pydantic import BaseModel, ConfigDict, Field

# Field literal type
AdAssetVideoField = Literal[
    "caption_ids",
    "id",
    "source_image_url",
    "tag",
    "thumbnail_hash",
    "thumbnail_source",
    "thumbnail_url",
    "url",
    "url_tags",
    "video_id",
    "video_name",
]


class AdAssetVideoFields(BaseModel):
    """Pydantic model for AdAssetVideo fields."""

    model_config = ConfigDict(populate_by_alias=True, extra="forbid")

    caption_ids: list[str] = Field(None, alias="caption_ids")
    id: str = Field(None, alias="id")
    source_image_url: str = Field(None, alias="source_image_url")
    tag: str = Field(None, alias="tag")
    thumbnail_hash: str = Field(None, alias="thumbnail_hash")
    thumbnail_source: str = Field(None, alias="thumbnail_source")
    thumbnail_url: str = Field(None, alias="thumbnail_url")
    url: str = Field(None, alias="url")
    url_tags: str = Field(None, alias="url_tags")
    video_id: str = Field(None, alias="video_id")
    video_name: str = Field(None, alias="video_name")
