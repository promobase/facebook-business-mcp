"""Code generated from Facebook API specs - DO NOT EDIT MANUALLY."""

from __future__ import annotations

from datetime import datetime
from enum import Enum
from typing import TYPE_CHECKING, Any, Literal

from pydantic import BaseModel, ConfigDict, Field

# Field literal type
AdCampaignGroupStatsField = Literal[
    "actions",
    "campaign_group_id",
    "clicks",
    "end_time",
    "impressions",
    "inline_actions",
    "social_clicks",
    "social_impressions",
    "social_spent",
    "social_unique_clicks",
    "social_unique_impressions",
    "spent",
    "start_time",
    "unique_clicks",
    "unique_impressions",
]


class AdCampaignGroupStatsFields(BaseModel):
    """Pydantic model for AdCampaignGroupStats fields."""

    model_config = ConfigDict(populate_by_alias=True, extra="forbid")

    actions: dict[str, int] = Field(None, alias="actions")
    campaign_group_id: str = Field(None, alias="campaign_group_id")
    clicks: int = Field(None, alias="clicks")
    end_time: datetime = Field(None, alias="end_time")
    impressions: int = Field(None, alias="impressions")
    inline_actions: dict[str, int] = Field(None, alias="inline_actions")
    social_clicks: int = Field(None, alias="social_clicks")
    social_impressions: int = Field(None, alias="social_impressions")
    social_spent: int = Field(None, alias="social_spent")
    social_unique_clicks: int = Field(None, alias="social_unique_clicks")
    social_unique_impressions: int = Field(None, alias="social_unique_impressions")
    spent: int = Field(None, alias="spent")
    start_time: datetime = Field(None, alias="start_time")
    unique_clicks: int = Field(None, alias="unique_clicks")
    unique_impressions: int = Field(None, alias="unique_impressions")
