"""Code generated from Facebook API specs - DO NOT EDIT MANUALLY."""

from __future__ import annotations

from datetime import datetime
from enum import Enum
from typing import TYPE_CHECKING, Any, Literal

from pydantic import BaseModel, ConfigDict, Field

if TYPE_CHECKING:
    from .group import GroupFields


class unifiedthreadmessages_source_enum_param(str, Enum):
    """unifiedthreadmessages_source_enum_param enum values."""

    ALL = "ALL"
    PARTICIPANTS = "PARTICIPANTS"


# Field literal type
UnifiedThreadField = Literal[
    "can_reply",
    "folder",
    "former_participants",
    "id",
    "is_subscribed",
    "link",
    "linked_group",
    "message_count",
    "name",
    "participants",
    "scoped_thread_key",
    "senders",
    "snippet",
    "subject",
    "unread_count",
    "updated_time",
    "wallpaper",
]


class UnifiedThreadFields(BaseModel):
    """Pydantic model for UnifiedThread fields."""

    model_config = ConfigDict(populate_by_alias=True, extra="forbid")

    can_reply: bool = Field(None, alias="can_reply")
    folder: str = Field(None, alias="folder")
    former_participants: dict[str, Any] = Field(None, alias="former_participants")
    id: str = Field(None, alias="id")
    is_subscribed: bool = Field(None, alias="is_subscribed")
    link: str = Field(None, alias="link")
    linked_group: GroupFields = Field(None, alias="linked_group")
    message_count: int = Field(None, alias="message_count")
    name: str = Field(None, alias="name")
    participants: dict[str, Any] = Field(None, alias="participants")
    scoped_thread_key: str = Field(None, alias="scoped_thread_key")
    senders: dict[str, Any] = Field(None, alias="senders")
    snippet: str = Field(None, alias="snippet")
    subject: str = Field(None, alias="subject")
    unread_count: int = Field(None, alias="unread_count")
    updated_time: datetime = Field(None, alias="updated_time")
    wallpaper: str = Field(None, alias="wallpaper")


class UnifiedThreadGetMessagesParams(BaseModel):
    """Parameters for UnifiedThread.get_messages()."""

    model_config = ConfigDict(extra="forbid")
    source: unifiedthreadmessages_source_enum_param | None = Field(
        None, description="source parameter"
    )
