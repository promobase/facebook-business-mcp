"""Code generated by PromoBase script - DO NOT EDIT MANUALLY."""

from __future__ import annotations

from typing import Any, Literal

from pydantic import BaseModel, Field

AdAccountAAASimilarCampaignsField = Literal["similar_campaign_limit", "similar_campaigns_info", "used_campaign_slots"]


class AdAccountAAASimilarCampaignsFields(BaseModel):
    """Pydantic model for AdAccountAAASimilarCampaigns fields."""

    similar_campaign_limit: int | None = Field(None, alias="similar_campaign_limit")
    similar_campaigns_info: list[dict[str, dict[str, Any]]] | None = Field(None, alias="similar_campaigns_info")
    used_campaign_slots: int | None = Field(None, alias="used_campaign_slots")

    class Config:
        populate_by_name = True
        extra = 'forbid'