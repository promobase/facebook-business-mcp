"""
Auto-generated MCP server for Facebook FundingSourceDetails.
DO NOT EDIT MANUALLY.
"""

from typing import Any, Optional

from facebook_business.adobjects.fundingsourcedetails import FundingSourceDetails
from fastmcp import FastMCP

# Initialize FastMCP server
mcp = FastMCP("facebook-fundingsourcedetails")


# CRUD Operations


@mcp.tool()
async def api_create_fundingsourcedetails(
    fundingsourcedetails_id: str,
    parent_id: Optional[Any] = None,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = FundingSourceDetails(fbid=fundingsourcedetails_id).api_create(
        parent_id=parent_id,
        fields=fields,
        params=params,
    )

    return result


@mcp.tool()
async def api_delete_fundingsourcedetails(
    fundingsourcedetails_id: str,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = FundingSourceDetails(fbid=fundingsourcedetails_id).api_delete(
        fields=fields,
        params=params,
    )

    return result


@mcp.tool()
async def api_get_fundingsourcedetails(
    fundingsourcedetails_id: str,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = FundingSourceDetails(fbid=fundingsourcedetails_id).api_get(
        fields=fields,
        params=params,
    )

    return result


@mcp.tool()
async def api_update_fundingsourcedetails(
    fundingsourcedetails_id: str,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = FundingSourceDetails(fbid=fundingsourcedetails_id).api_update(
        fields=fields,
        params=params,
    )

    return result


# Export the server
fundingsourcedetails_server = mcp
