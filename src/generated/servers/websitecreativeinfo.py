"""
Auto-generated MCP server for Facebook WebsiteCreativeInfo.
DO NOT EDIT MANUALLY.
"""

from typing import Any, Optional

from facebook_business.adobjects.websitecreativeinfo import WebsiteCreativeInfo
from fastmcp import FastMCP

# Initialize FastMCP server
mcp = FastMCP("facebook-websitecreativeinfo")


# CRUD Operations


@mcp.tool()
async def api_create_websitecreativeinfo(
    websitecreativeinfo_id: str,
    parent_id: Optional[Any] = None,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = WebsiteCreativeInfo(fbid=websitecreativeinfo_id).api_create(
        parent_id=parent_id,
        fields=fields,
        params=params,
    )

    return result


@mcp.tool()
async def api_delete_websitecreativeinfo(
    websitecreativeinfo_id: str,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = WebsiteCreativeInfo(fbid=websitecreativeinfo_id).api_delete(
        fields=fields,
        params=params,
    )

    return result


@mcp.tool()
async def api_get_websitecreativeinfo(
    websitecreativeinfo_id: str,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = WebsiteCreativeInfo(fbid=websitecreativeinfo_id).api_get(
        fields=fields,
        params=params,
    )

    return result


@mcp.tool()
async def api_update_websitecreativeinfo(
    websitecreativeinfo_id: str,
    fields: list[str] = [],
    params: dict[str, Any] = {},
) -> dict[str, Any]:
    result = WebsiteCreativeInfo(fbid=websitecreativeinfo_id).api_update(
        fields=fields,
        params=params,
    )

    return result


# Export the server
websitecreativeinfo_server = mcp
