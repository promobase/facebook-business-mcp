        ...
        for key, value in zip(dict_node.keys, dict_node.values, strict=False):
            if isinstance(key, ast.Constant):
                ...
                if isinstance(value, ast.Attribute):
                    if isinstance(value.value, ast.Attribute) and isinstance(value.value.value, ast.Name):
...
class PydanticModelGenerator:
    ...
    def _generate_param_model(self, class_name: str, method_info: dict) -> Optional[str]:
        ...
        model_name = f"{class_name}{''.join(word.capitalize() for word in method_name.split('_'))}Params"
        ...
        if param_type.endswith("_enum"):
            ...
            if param_type in enums:
                ...
                if "." in enum_ref:
                    ...
                    return f"str"  # For now, use str; could be improved to use actual enum
            return "str"
...
def main():
    ...
    # Process each AdObject file
    adobject_files = parser.find_adobject_files()

    # Focus on key objects first
    priority_objects = ["adaccount", "campaign", "adset", "ad", "adcreative", "customaudience"]

    for file_path in adobject_files:
        if any(obj in file_path.stem for obj in priority_objects):
    ...
    with open(init_file, "w") as f:
        ...
        f.write('"""Auto-generated Pydantic models for Facebook Business SDK."""\n\n')
        # Collect all module names and sort them
        module_names = sorted([file_path.stem for file_path in output_dir.glob("*_models.py")])
